	...
	public void onClick(View v) {
		...
		case R.id.mSubmit:
			if (TextUtils.isEmpty(metName.getText().toString())) {
				ToastUtil.ShowCentre(RegisterCreatActivity.this, "请输入用户名");
				return;
			}
			
			if (TextUtils.isEmpty(metPhone.getText().toString())) {
				ToastUtil.ShowCentre(RegisterCreatActivity.this, "请输入手机号码");
				return;
			}

			
			if (!isPhone(metPhone.getText().toString())) {
				ToastUtil.ShowCentre(RegisterCreatActivity.this, "你输入的手机号码格式不正确");
				return;
			}

			
			if (TextUtils.isEmpty(metPswd.getText().toString())) {
				ToastUtil.ShowCentre(RegisterCreatActivity.this, "请输入密码");
				return;
			}
